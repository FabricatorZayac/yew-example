...
async fn fetch_get(url: &str) -> Result<String, gloo_net::Error> {
...
impl Component for Hello {
        ...
        match msg {
            ...
            Msg::FetchHello => {
                ctx.link().send_future(async {
                    match fetch_get("http://localhost:8000/hello").await {
            ...
            Msg::FetchAsyncDelay => {
                ...
                ctx.link().send_future(async move {
                    match fetch_get(format!("http://localhost:8000/hello/delay/{}", delay).as_str()).await {
                        Ok(hello) => Msg::SetFetchState(FetchState::Success(hello)),
                        Err(err) => Msg::SetFetchState(FetchState::Failed(err)),
                    }
                });
            ...
            Msg::FetchHelloName => {
                ...
                let name = if input.is_empty() { "Jimmy".to_owned() } else { input };

                ctx.link().send_future(async move {
                    match fetch_get(format!("http://localhost:8000/hello/{}", name).as_str()).await {
                        Ok(hello) => Msg::SetFetchState(FetchState::Success(hello)),
                        Err(err) => Msg::SetFetchState(FetchState::Failed(err)),
                    }
                });
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        let res = match &self.fetch_state {
